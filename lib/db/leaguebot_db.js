...
class LeagueBotDb extends BaseDb {
  ...
  async requestLink(guild, type) {
    if (!(type in LeagueBotDb.LINK_TYPES)) return Promise.reject(`Unknown link type ${guild.id}`)
  ...
  async acceptLink(guild, link_id) {  
    ...
    let admin = link_request.type == LeagueBotDb.LINK_TYPES.ADMIN ? link_request.guild_id : guild.id
    let player = link_request.type == LeagueBotDb.LINK_TYPES.PLAYER ? link_request.guild_id : guild.id
